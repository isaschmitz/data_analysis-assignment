...
from correlation_trends.seasonal import (
    deseasonalize,
...
def test_deseasonalize_returns_flat_climatology_and_metadata() -> None:
    da = _monthly_series(years=3, amp=4.0, mean=12.0)
    deseasonalized, climatology = deseasonalize(da, min_years=2)
    assert climatology.sizes["month"] == 12
    assert float(deseasonalized.mean()) == pytest.approx(float(da.mean()))
    residual = deseasonalized.groupby("TIME.month").mean()
    assert residual.max().item() - residual.min().item() < 1e-6
    assert "deseasonalized" in deseasonalized.attrs
